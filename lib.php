...
function theme_eskuela_get_main_scss_content($theme) {
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/eskuela/scss/pre.scss');
	
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/eskuela/scss/post.scss');
...
function theme_eskuela_update_settings_images($settingname) {
    ...
    if ($file = $fs->get_file_by_hash(sha1($fullpath))) {
        ...
        $pathname = $CFG->dataroot . '/pix_plugins/theme/eskuela/' . $settingname . '.' . $extension;

        // This pattern matches any previous files with maybe different file extensions.
        $pathpattern = $CFG->dataroot . '/pix_plugins/theme/eskuela/' . $settingname . '.*';

        // Make sure this dir exists.
        @mkdir($CFG->dataroot . '/pix_plugins/theme/eskuela/', $CFG->directorypermissions, true);
